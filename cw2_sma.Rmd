...
## Function to  install and load all required libraries
```{r}
load_required_libraries <- function(pkg){
   for( i in pkg ){
...
load_required_libraries(c("twitteR" , "rtweet" , "ROAuth", "syuzhet", "plotly", "tm", "wordcloud","textmineR","topicmodels","plyr","stringr","ggplot2","scales","base64enc","gridExtra","ggpubr","maps","anytime"  ))
```


## If all libraries are already installed
...
connect_to_twitter <- function(){
  ...
  # Another account is used for premium API, If the same keys were used for both libraries at once, might cause token issues.
  appname_premium <- ""
  consumerKey_premium<-	""
  consumerSecret_premium<-""
  accessToken_premium<-""
  accessSecret_premium<-""
  
  # authenticate with Twitter for rtweet
  twitter_token <- create_token(
  app = appname_premium,
  consumer_key = consumerKey_premium,
  consumer_secret = consumerSecret_premium,
  access_token = accessToken_premium,
  access_secret = accessSecret_premium)
...
Search for multiple Islam related keywords and bind the results together.
```{r, eval=FALSE}
twitter_token = connect_to_twitter()


# search for tweets by keyword "MUSLIM"
tweets<-searchTwitter("muslim", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
tweets_muslim<-twListToDF(tweets) # put tweets in a data frame


# search for tweets by keyword "ISLAM"
tweets<-searchTwitter("islam", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
tweets_islam<-twListToDF(tweets)


# search for tweets by keyword "Arab"
tweets<-searchTwitter("arab", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
tweets_arab<-twListToDF(tweets)


# search for tweets by keyword "Muhammad"
tweets<-searchTwitter("muhammad", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
...
Search for multiple CHristianity related keywords and bind the results together.
```{r}
connect_to_twitter()

# search for tweets by keyword
tweets<-searchTwitter("christian", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
# put tweets in a data frame
tweets_christian<-twListToDF(tweets)

tweets<-searchTwitter("christ", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
# put tweets in a data frame
tweets_christ<-twListToDF(tweets)

tweets<-searchTwitter("catholic", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
# put tweets in a data frame
tweets_catholic<-twListToDF(tweets)

tweets<-searchTwitter("church", n=1000, lang="en", since=NULL, until=NULL, locale=NULL, geocode=NULL, sinceID=NULL, maxID=NULL,
                                resultType=NULL, retryOnRateLimit=120)
# put tweets in a data frame
...
# generate wordcloud
wordcloud(muslim_corpus,min.freq = 10,scale=c(3,.5),random.order=TRUE,colors=brewer.pal(6, "Dark2"),random.color = TRUE,max.words = 20)
wordcloud(christian_corpus,min.freq = 10,colors=brewer.pal(6, "Dark2"),scale=c(3,.5),random.order=TRUE,random.color = TRUE,max.words = 20)
```


## Sentiment Analysis
This code will perform the sentiment analysis comparison of Islam tweets and Christian tweet, by using the get_nrc_sentiment function to do so. This bit also uses the dataframes read from the CSV files in the word cloud section.
```{r}
...
Sentimentscores_muslim <- Sentimentscores_muslim[c(1:8),]

Viz_sa_muslim <- ggplot(data=Sentimentscores_muslim,aes(x=sentiment,y=Score))+geom_bar(aes(fill=sentiment),stat = "identity")+
 ...
 Sentimentscores_christian <- Sentimentscores_christian[c(1:8),]
 
 Viz_sa_christian <- ggplot(data=Sentimentscores_christian,aes(x=sentiment,y=Score))+geom_bar(aes(fill=sentiment),stat = "identity")+
   theme(legend.position="none")+
   xlab("Sentiments")+ylab("scores")+ggtitle("Sentiments of people behind the tweets on Christian")
 
 
 ggarrange( Viz_sa_christian  + theme(legend.position="none"),
          Viz_sa_muslim + theme(legend.position="none"),
          common.legend = TRUE,
          ncol = 1, nrow = 2)
```
...
## Emotion Plot
This section will perform the emotion plot coparison of Islam tweets and Christian tweets. It extract the emotion information from the results of the get_nrc_sentiment function. This bit also uses the dataframes read from the CSV files in the word cloud section.
```{r}
# Muslim Tweets
emotions_muslim <- get_nrc_sentiment(muslim_text)
emo_bar_muslim = colSums(emotions_muslim)
emo_sum_muslim = data.frame(count=emo_bar_muslim, emotion=names(emo_bar_muslim))
emo_sum_muslim$emotion = factor(emo_sum_muslim$emotion, levels=emo_sum_muslim$emotion[order(emo_sum_muslim$count, decreasing = TRUE)])


emo_sum_muslim <- emo_sum_muslim[1:8,]
emo_sum_muslim$percent<-(emo_sum_muslim$count/sum(emo_sum_muslim$count))*100


Viz_ed_muslim<- ggplot(data=emo_sum_muslim,aes(x=emotion,y=percent))+geom_bar(aes(fill=emotion),stat = "identity")+
   theme(legend.position="none")+ylab("scores(%)")+ggtitle("Distribution of emotion categories on  Muslim Tweets")

# # Chritian Tweets
emotions_christian <- get_nrc_sentiment(christian_text)
emo_bar_christian = colSums(emotions_christian)
emo_sum_christian = data.frame(count=emo_bar_christian, emotion=names(emo_bar_christian))
emo_sum_christian$emotion = factor(emo_sum_christian$emotion, levels=emo_sum_christian$emotion[order(emo_sum_christian$count, decreasing = TRUE)])

emo_sum_christian <- emo_sum_christian[1:8,]
emo_sum_christian$percent<-(emo_sum_christian$count/sum(emo_sum_christian$count))*100

Viz_ed_christian<- ggplot(data=emo_sum_christian,aes(x=emotion,y=percent))+geom_bar(aes(fill=emotion),stat = "identity")+
   theme(legend.position="none")+ ylab("scores(%)")+ggtitle("Distribution of emotion categories on Christian Tweets")


ggarrange( Viz_ed_christian  + theme(legend.position="none"),
          Viz_ed_muslim + theme(legend.position="none"),
          common.legend = TRUE,
          ncol = 1, nrow = 2)
```
...
## Topic Model
This next section will perform topic modelling using LDA for both Islam related tweets and Christianity related tweets. This bit also uses the dataframes read from the CSV files in the word cload section.
```{r}
...
pos <- readLines("input/positive_words.txt")
neg <- readLines("input/negative_words.txt")
...
for( i in month_days) {
    ...
    # Prepare parameters
    t_rnd = sample(1:(24*60*60), 1) # Get a random offest in random time in each day : i
    toDate <- format(Sys.time() - 60 * 60 * 24 * i - t_rnd, "%Y%m%d%H%M")
    
    # Apply search for the whole month usign the the word "muslims".
    # Using Enviroment : "dev30" (30 day premium Twtter API)
    tweets_tmp <- search_30day("muslims", n = 100 ,env_name = "dev30", parse=TRUE, toDate = toDate, token = twitter_token)
    
    # Append to the list 'tweets' if not there, create it !.
    if(exists("tweets_30_days")) {
        tweets_30_days <- rbind(tweets_30_days, tweets_tmp)
    } else {
        tweets_30_days <- tweets_tmp
    }
  
}

# Export the results as CSV.
write_as_csv(tweets_30_days, '30_day_tweets.csv')
```
...
wordcloud(tweet_clean, random.order=0.5,max.words=30, colors=brewer.pal(6, "Dark2"), random.color = TRUE,min.freq = 20)
...
### Location Analysis
Here we get positive, negative and neutral sentiment scores for each of the cities.
```{r, eval=FALSE}
...
### Visualisation
We then plot the scores for all ten cities into one plot.
```{r eval=FALSE}
text_size <- 12
axis_text_size<- 10
...
# Function to get tweets given a year and a query with keywords.
get_tweets_by_year <- function(year , keyword) {
  ...
  for(i in 1:5) {
    t_rnd = sample(1:(364*24*60*60), 1)
    toDate <- format(anytime(year_unix + t_rnd), "%Y%m%d%H%M")
    
    # "lang:en muslims"
    tweets_tmp <- search_fullarchive(keyword, n = 50, env_name = "dev", toDate = toDate, parse=TRUE,token = twitter_token)
...
# Function to Get Sentiment score for each year using the formala:
# Match_pos - Match_neg
score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
{
  # Parameters
...
# Function to clean and prepare the tweets.
clean_tweets <- function(csv_file) {
  
# import your data set to analyse,
# ensure it is in the same directory as your code, otherwise you need to add the path
dataset_csv <- read_twitter_csv(csv_file)
tweet_year_clean <- dataset_csv['text']

# convert text to lowercase
tweet_year_clean<-tolower(tweet_year_clean)

# get rid of problem characters
tweet_year_clean <- sapply(tweet_year_clean,function(row) iconv(row, "latin1", "ASCII", sub=""))

# remove punctuation, digits, special characters etc
tweet_year_clean = gsub("&amp", "", tweet_year_clean)
tweet_year_clean= gsub("(RT|via)((?:\\b\\W*@\\w+)+)", "", tweet_year_clean)
tweet_year_clean = gsub("@\\w+", "", tweet_year_clean)
tweet_year_clean= gsub("[[:punct:]]", "", tweet_year_clean)
tweet_year_clean = gsub("[[:digit:]]", "", tweet_year_clean)
tweet_year_clean = gsub("http\\w+", "", tweet_year_clean)
tweet_year_clean = gsub("[ \t]{2,}", "", tweet_year_clean)
tweet_year_clean= gsub("^\\s+|\\s+$", "", tweet_year_clean) 


# get rid of unnecessary spaces
tweet_year_clean <- str_replace_all(tweet_year_clean," "," ")

# get rid of URLs
#tweets.df <- str_replace_all(tweets.df, "http://t.co/[a-z,A-Z,0-9]*{8}","")

# take out the retweet header (there is only one)
tweet_year_clean <- str_replace(tweet_year_clean,"RT @[a-z,A-Z]*: ","")

# get rid of hashtags
tweet_year_clean <- str_replace_all(tweet_year_clean,"#[a-z,A-Z]*","")

# get rid of references to other screen names
tweet_year_clean <- str_replace_all(tweet_year_clean,"@[a-z,A-Z]*","") 
  
return(tweet_year_clean)

}

```

### Vizuaise Islamophobia over the years
```{r,eval=FALSE}
# Repeat for each year, 2017 - 2020
tweet_year <- get_tweets_by_year(2015 , "lang:en muslims")

tweet_year_clean <- clean_tweets("2015-tweets.csv")

# using the function to calculate sentiment score for the year
scores_year <- score.sentiment(tweet_year_clean, pos.txt, neg.txt, .progress='text')

# the value is recorded for the given year.
scores_year$score_chr <- ifelse(scores_year$score < 0,'Negtive', ifelse(scores_year$score > 0, 'Positive', 'Neutral'))
View(scores_year)
